...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['outcome'] = filtered_df['class'].apply(lambda x: 'Successful Launch' if (x == 1) else 'Failed Launch')
        filtered_df['counts'] = 1
        return px.pie(filtered_df, values='counts', 
                    names='outcome', hover_data=['class'], labels={'class':'Success'}, 
...
def get_scatter_chart(entered_site, slider):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['outcome'] = filtered_df['class'].apply(lambda x: 'Successful Launch' if (x == 1) else 'Failed Launch')
        filtered_df['counts'] = 1
        return px.scatter (filtered_df, x='Payload Mass (kg)', y='class', color_discrete_sequence=px.colors.sequential.RdBu,
